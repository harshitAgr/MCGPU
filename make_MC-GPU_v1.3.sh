#!/bin/bash
# 
#   ** Simple script to compile the code MC-GPU v1.3 with CUDA 13 **
#
#      The installations paths to the CUDA toolkit (http://www.nvidia.com/cuda) and the MPI
...
# -- Compile GPU code for compute capability 7.5, 8.0, and 9.0, with MPI:

echo " "
echo " -- Compiling MC-GPU v1.3 with CUDA 13 for compute capability 7.5, 8.0, and 9.0 (64 bits), with MPI:"
...
echo " "
echo "nvcc MC-GPU_v1.3.cu -o MC-GPU_v1.3.x -m64 -O3 -use_fast_math -DUSING_CUDA -DUSING_MPI -I. -I/usr/local/cuda/include -I/usr/include/openmpi -L/usr/lib/ -lmpi -lz --ptxas-options=-v -gencode=arch=compute_75,code=sm_75 -gencode=arch=compute_80,code=sm_80 -gencode=arch=compute_90,code=sm_90"
nvcc MC-GPU_v1.3.cu -o MC-GPU_v1.3.x -m64 -O3 -use_fast_math -DUSING_CUDA -DUSING_MPI -I. -I/usr/local/cuda/include -I/usr/include/openmpi -L/usr/lib/ -lmpi -lz --ptxas-options=-v -gencode=arch=compute_75,code=sm_75 -gencode=arch=compute_80,code=sm_80 -gencode=arch=compute_90,code=sm_90
